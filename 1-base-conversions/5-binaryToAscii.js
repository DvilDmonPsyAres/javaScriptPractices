...
const binaryToAscii = str => {
  const binaryArray = binaryStringToArray(str); // Convert binary string to an array of 8-bit strings
  let asciiString = '';

  for (let i = 0; i < binaryArray.length; i++) {
    const binaryValue = binaryArray[i];
    const decimalValue = parseInt(binaryValue, 2); // Convert binary string to decimal

    // Convert decimal value to ASCII character and append to the result string
